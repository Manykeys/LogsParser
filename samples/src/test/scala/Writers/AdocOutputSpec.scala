...
class AdocOutputSpec extends AnyFunSuite with Matchers {
  ...
  test("AdocOutput should render correctly with from and to dates") {
    val rendered =
      AdocOutput.render(dummyStats, fromDate, toDate).unsafeRunSync()
    ...
    rendered should include("| /index.html | 100")
    rendered should include(" /about.html | 50")
    rendered should include("| 200 | OK | 140")
    rendered should include(" 404 | Not Found | 10")
  }

  test("AdocOutput should render correctly with no from and to dates") {
    val rendered = AdocOutput.render(dummyStats, None, None).unsafeRunSync()
